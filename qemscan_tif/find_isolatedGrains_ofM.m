% this script is to find isolated grains of minerals M.
...
for H = 1:2
    ...
    %%% are touching the other minerals. You are finding isolated grains
    %%% and you don't care about the mineralogy
    ...
    Mineral = min_col(2,:)
    M_r = (Irgb(:,:,1)==Mineral(1));
    M_g = (Irgb(:,:,2)==Mineral(2));
    M_b = (Irgb(:,:,3)==Mineral(3));
    IrgbM = logical(M_r.*M_g.*M_b);
        ...
        for i = 2:num_r-1
            ...
            if IrgbM==1
                ...
                for k = 1:8
                    ...
                    j_in = j+ar_j(k);
                    pix(k,:) = [i_in,j_in];
                    I_mtx_k(k) = I_mtxM(i_in,j_in);

                end  %% this is where you get stuck for the nigth before leaving, pick up on this tomorrow or monday.

                elG = find(I_mtx_k>0);

                if elG
    %                 I_mtx_k(elG(1))
                    I_mtxM(i,j) = I_mtx_k(elG(1));
    %                 keyboard
                elseif isempty(elG)
                    I_mtxM(i,j) = count;
                end
