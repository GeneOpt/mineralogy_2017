...
% "find_clustersOfMinerals". They are concatenated between files using the
% concatenate_dot_Matfile.m script. The variables are now clustM_c, which
% is an object with the mineral size in one column and the associated grain
...
%run the conatenate script if you need to do this. 
...
%         keyboard
    ...
    ylabel('Normalized size fraction')
    xlabel('sqrt [Grain size (pixels)]')
    grid on
