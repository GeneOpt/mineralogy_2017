...
% run read_in_qemscan
% 
% save('tempDatDump.mat','var_ss','var_BMMP_F','dat_BMMP_F','dat_BMMP_S',...
%     'dat_SMPS_S','dat_SMPG_S','varMD','datMD','dat_sed','var_sed')
...
%% samples for rock fragments
% here are the samples for the rocks
...
% lab = lab(keepLab-1)
%%
% samples for sediment fragments
% samples = {'GL 01','GL 02','GL 03','GL 04','GL 05','GL 06','GL 07','GL 08',...
%     'GL 09','GL 10','GL 11','GL 13','GL 14','GL 15','GL 16','GL 17',...
%     'GL 18','GL 19','GL 20','GL 21','13BH50'} this is the entire list
samples = {'GL 01','GL 02','GL 03','GL 04','GL 05','GL 06','GL 07',...
    'GL 08','GL 09','GL 10','GL 11','GL 13','GL 14','GL 15','GL 16','GL 17',...
    'GL 18','GL 19','GL 20','GL 21'}

for i = 1:length(samples)
    keepLab(i) = find(strcmp(var_sed(:,1),samples(i)))
end

st = strcmp(var_sed(keepLab,3),'S')
nst = strcmp(var_sed(keepLab,3),'NS')
gt = var_sed(keepLab,3)
mx = strcmp(var_sed(keepLab,4),'MX')
ms = strcmp(var_sed(keepLab,4),'MS')
ct = var_sed(keepLab,4)
group = var_sed(keepLab,5)
groupLab = dat_sed(keepLab-1,5)
...
%     dat1 = dat_SMPS_S(3:4:end,:)
%     dat = dat1(keepLab-1,:)
    
    dat = dat_BMAP_S(keepLab-1,:)
    ...
    %     ii = i+(a-1);
        ...
        [p,anovatab,stats] = anova1(y(strcmp('MS',ct)),gt(strcmp('MS',ct)),'off');
        [c,m,h,nms] = multcompare(stats,'ctype','hsd','alpha',0.1);
        group2_p(3,i) = c(:,6);
        title(['MS gt ' vars(i)]);
        keyboard
        %%%plutonic surge versus non surge
        [p,anovatab,stats] = anova1(y(strcmp('MX',ct)),gt(strcmp('MX',ct)),'off');
        [c,m,h,nms] = multcompare(stats,'ctype','hsd','alpha',0.1);
        group2_p(2,i) = c(:,6);
        title(['MX gt ' vars(i)]);
        keyboard
...
%         title(minerals(i));
        ...
        [p,anovatab,stats] = anova1(y,ct,'off');
        [c,m,h,nms] = multcompare(stats,'ctype','hsd','alpha',0.1);
        rtA_p(:,i) = c(:,6);
        title(vars(i));
        keyboard
...
%     if vlt(i)
    ...
    p1 = plot(ones(1,sum(mx.*st))*4,y(logical(mx.*st)),'ko','markerfacecolor','r','markersize',mks)
    p2 = plot(ones(1,sum(mx.*nst))*3,y(logical(mx.*nst)),'k^','markerfacecolor','r','markersize',mks)
    p3 = plot(ones(1,sum(ms.*st))*2,y(logical(ms.*st)),'ko','markerfacecolor',[0.7 0.7 0.7],'markersize',mks)
    p4 = plot(ones(1,sum(ms.*nst))*1,y(logical(ms.*nst)),'k^','markerfacecolor',[0.7 0.7 0.7],'markersize',mks)
    grid on
%     text(p_r+1+0.1,y,lab,'fontsize',fs-2)
    text(groupLab+0.1,y,lab)
    set(gca,'XTick',1:4,'XTickLabel',{'MSNS','MSS','MXNS','MXS'})
    xlim([0 5])
...
i1 = find(strcmp(mins,str1))
i2 = find(strcmp(mins,str2))
...
%         set(gcf,'visible','off')
        hold on
        p1 = plot(x(logical(mx.*st)),y(logical(mx.*st)),'ko','markerfacecolor','r','markersize',mks);
        p2 = plot(x(logical(mx.*nst)),y(logical(mx.*nst)),'k^','markerfacecolor','r','markersize',mks);
        p3 = plot(x(logical(ms.*st)),y(logical(ms.*st)),'ko','markerfacecolor',[0.7 0.7 0.7],'markersize',mks);
        p4 = plot(x(logical(ms.*nst)),y(logical(ms.*nst)),'k^','markerfacecolor',[0.7 0.7 0.7],'markersize',mks);
%         p4 = plot(x(logical(Gb_r.*nst)),y(logical(Gb_r.*nst)),'k^','markerfacecolor',[0 0 0],'markersize',ms);
%       
        text(x(ms,:),y(ms,:),lab(ms))
        text(x(mx,:),y(mx,:),lab(mx))
%         text(x(Gb_r,:),y(Gb_r,:),lab(Gb_r))
        xlm = get(gca,'xlim');
        ylm = get(gca,'ylim');
        
        cMS = corr(x(ms),y(ms));
        pMS = polyfit(x(mx),y(mx),1);
        yMS = pMS(1).*xlm + pMS(2);
        plot(xlm,yMS,'k')
...
set(gca,'xtick',1:length(vars),'xticklabel',vars(els))
rotateXLabels(gca,60)
set(gca,'fontsize',14)
...
% stM = logical(st(mx))
% nstM = logical(nst(mx))
% p3 = plot(x(stM),y(stM),'ko','markerfacecolor','r','markersize',mks);
% p4 = plot(x(nstM),y(nstM),'k^','markerfacecolor','r','markersize',mks);
% text(x,y,lab(mx))
p1 = plot(x(logical(mx.*st)),y(logical(mx.*st)),'ko','markerfacecolor','r','markersize',mks);
p2 = plot(x(logical(mx.*nst)),y(logical(mx.*nst)),'k^','markerfacecolor','r','markersize',mks);
p3 = plot(x(logical(ms.*st)),y(logical(ms.*st)),'ko','markerfacecolor',[0.7 0.7 0.7],'markersize',mks);
...
ylabel('PC 3')
xlabel('PC 1')
% legend([p1 p2 p3 p4],{'MXS','MXNS','MSS','MSNS'},'location','northwest')
set(gca,'fontsize',18)
% savePDFfunction(f1,'seds_PC1_3size1_4')
...
ylabel('PC 1')
set(gca,'fontsize',14)
savePDFfunction(fPC1,'seds_16_4_PC1_allMins_bySMPSS')
...
p3 = plot(ones(1,sum(ms.*st))*2,y(logical(ms.*st)),'ko','markerfacecolor',[0.7 0.7 0.7],'markersize',mks)
p4 = plot(ones(1,sum(ms.*nst))*1,y(logical(ms.*nst)),'k^','markerfacecolor',[0.7 0.7 0.7],'markersize',mks)
set(gca,'XTick', 1:4,'XTickLabel',{'MSS','MSNS','MXS','MXNS'})
set(gca,'XTick', 1:4,'XTickLabel',{'MSS','MSNS','MXS','MXNS'})
xlim([0 5])
text(groupLab(:)+0.1,y,lab(:))
grid on
...
subplot(2,1,2)
hold on
p1 = plot(ones(1,length(x(stM))),x(stM),'ko','markerfacecolor','r','markersize',mks);
text(ones(1,length(x(stM)))+0.1,x(stM),lab(logical(st.*mx)));
p2 = plot(ones(1,length(x(nstM)))*2,x(nstM),'k^','markerfacecolor','r','markersize',mks);
text(ones(1,length(x(nstM)))*2+0.1,x(nstM),lab(logical(nst.*mx)));
set(gca,'XTick', 1:2,'XTickLabel',{'MXS','MXNS'})
xlim([0 3])
grid on
